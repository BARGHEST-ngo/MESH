...
func (device *Device) SendHandshakeCookie(
	...
	// DEBUG: Nil checks before Send
	if device == nil {
		panic("SendHandshakeCookie: device is nil")
	}
	if device.net.bind == nil {
		panic("SendHandshakeCookie: device.net.bind is nil")
	}
	if initiatingElem == nil {
		panic("SendHandshakeCookie: initiatingElem is nil")
	}
	if initiatingElem.endpoint == nil {
		panic("SendHandshakeCookie: initiatingElem.endpoint is nil")
	}

	device.net.bind.Send([][]byte{junkedHeader}, initiatingElem.endpoint, 0)
	return nil
}
...
func (device *Device) RoutineReadFromTUN() {
	defer func() {
		if r := recover(); r != nil {
			device.log.Errorf("RoutineReadFromTUN: PANIC RECOVERED: %v", r)
		}
		device.log.Errorf("Routine: TUN reader - stopped")
		device.state.stopping.Done()
		device.queue.encryption.wg.Done()
	}()

	device.log.Errorf("Routine: TUN reader - started")
	...
	device.log.Errorf("RoutineReadFromTUN: batchSize=%d, offset=%d", batchSize, offset)

	for i := range elems {
		device.log.Errorf("RoutineReadFromTUN: Creating outbound element %d", i)
		elems[i] = device.NewOutboundElement()
		bufs[i] = elems[i].buffer[:]
	}
	device.log.Errorf("RoutineReadFromTUN: All elements created successfully")
	...
	device.log.Errorf("RoutineReadFromTUN: Entering main loop")
	for {
		// read packets
		device.log.Errorf("RoutineReadFromTUN: About to read from TUN device")
		device.log.Errorf("RoutineReadFromTUN: device.tun.device=%v", device.tun.device)
		if device.tun.device == nil {
			device.log.Errorf("RoutineReadFromTUN: ERROR - device.tun.device is nil!")
			break
		}
		count, readErr = device.tun.device.Read(bufs, sizes, offset)
		device.log.Errorf("RoutineReadFromTUN: Read %d packets, err=%v", count, readErr)
