...
func NewDevice(tunDevice tun.Device, bind conn.Bind, logger *Logger) *Device {
	logger.Verbosef("NewDevice: Creating new WireGuard device...")
	device := new(Device)

	// CRITICAL: Initialize AWG FIRST, before anything else that might trigger goroutines
	// This prevents race conditions where API calls try to access AWG before it's ready
	// NOTE: We initialize AWG fields individually to avoid Go runtime alignment issues
	// with the write barrier when assigning structs containing interfaces
	logger.Errorf("NewDevice: Initializing AWG structures...")

	// Create default magic headers for standard WireGuard (1, 2, 3, 4)
	logger.Errorf("NewDevice: Creating default magic headers...")
	defaultMagicHeaders := []awg.MagicHeader{
		awg.NewMagicHeaderSameValue(DefaultMessageInitiationType),
		awg.NewMagicHeaderSameValue(DefaultMessageResponseType),
		awg.NewMagicHeaderSameValue(DefaultMessageCookieReplyType),
		awg.NewMagicHeaderSameValue(DefaultMessageTransportType),
	}
	logger.Errorf("NewDevice: Default magic headers created successfully")

	// Initialize MagicHeaders fields individually to avoid alignment issues
	logger.Errorf("NewDevice: Setting MagicHeaders.Values...")
	device.awg.Cfg.MagicHeaders.Values = defaultMagicHeaders
	logger.Errorf("NewDevice: MagicHeaders.Values set successfully")

	logger.Errorf("NewDevice: Creating PRNG for MagicHeaders...")
	prng := awg.NewPRNG[uint32]()
	logger.Errorf("NewDevice: PRNG created successfully, calling SetRandomGenerator...")
	device.awg.Cfg.MagicHeaders.SetRandomGenerator(prng)
	logger.Errorf("NewDevice: SetRandomGenerator completed successfully")

	// Initialize JunkCreator and set AWG off
	logger.Errorf("NewDevice: Creating JunkCreator...")
	device.awg.JunkCreator = awg.NewJunkCreator(device.awg.Cfg)
	logger.Errorf("NewDevice: JunkCreator created successfully")
	device.awg.IsOn.SetTo(false)
	logger.Errorf("NewDevice: AWG IsOn set to false")

	// Verify AWG initialization succeeded
	if device.awg.Cfg.MagicHeaders.Values == nil {
		panic("FATAL: AWG MagicHeaders.Values is nil after initialization!")
	}
	logger.Errorf("NewDevice: AWG initialized successfully (MagicHeaders.Values len=%d, IsOn=%v)",
		len(device.awg.Cfg.MagicHeaders.Values), device.awg.IsOn.IsSet())

	// Now continue with normal device initialization
	logger.Errorf("NewDevice: Setting device state...")
	device.state.state.Store(uint32(deviceStateDown))
	device.closed = make(chan struct{})
	device.log = logger
	logger.Errorf("NewDevice: Basic initialization complete")

	logger.Errorf("NewDevice: Setting bind...")
	device.net.bind = bind
	logger.Errorf("NewDevice: Setting TUN device...")
	device.tun.device = tunDevice

	logger.Errorf("NewDevice: Getting MTU...")
	if device.tun.device == nil {
		device.log.Errorf("NewDevice: WARNING - TUN device is nil, using default MTU")
		...
		logger.Errorf("NewDevice: MTU set to %d", mtu)
	}

	logger.Errorf("NewDevice: Creating peers keyMap...")
	device.peers.keyMap = make(map[NoisePublicKey]*Peer)
	logger.Errorf("NewDevice: Initializing rate limiter...")
	device.rate.limiter.Init()
	logger.Errorf("NewDevice: Initializing index table...")
	device.indexTable.Init()

	logger.Errorf("NewDevice: Populating pools...")
	device.PopulatePools()
	logger.Errorf("NewDevice: Pools populated")

	// create queues
	logger.Errorf("NewDevice: Creating queues...")
	device.queue.handshake = newHandshakeQueue()
	device.queue.encryption = newOutboundQueue()
	device.queue.decryption = newInboundQueue()
	logger.Errorf("NewDevice: Queues created")

	// start workers
	logger.Errorf("NewDevice: Starting workers...")
	cpus := runtime.NumCPU()
	logger.Errorf("NewDevice: CPU count: %d", cpus)
	...
	logger.Errorf("NewDevice: Starting RoutineReadFromTUN...")
	go device.RoutineReadFromTUN()
	logger.Errorf("NewDevice: Starting RoutineTUNEventReader...")
	go device.RoutineTUNEventReader()
	logger.Errorf("NewDevice: All workers started, returning device")
...
func (device *Device) ProcessAWGPacket(size int, packet *[]byte, buffer *[MaxMessageSize]byte) (uint32, error) {
	// DEBUG: Defensive check - ensure MagicHeaders is initialized
	if device.awg.Cfg.MagicHeaders.Values == nil {
		return 0, fmt.Errorf("ProcessAWGPacket called but MagicHeaders not initialized")
	}
