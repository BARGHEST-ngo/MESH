...
func (b *backend) isConfigNonNilAndDifferent(rcfg *router.Config, dcfg *dns.OSConfig) bool {
	// Avoid reflect.DeepEqual on pointer-bearing types as it can trigger
	// bulkBarrierPreWrite errors on arm64 due to alignment issues.
	// Instead, use pointer equality checks which are safe.
	if rcfg == b.lastCfg && dcfg == b.lastDNSCfg {
