...
import type { V1Node, V1ListNodesResponse } from './openapi/types.gen'
...
//Exit routes (duplicated here to avoid circular import with useNodes.ts)
...
/**
 * Build a set of network names that have at least one approved exit node.
 */
...
export async function syncNetworkIsolationPolicy(): Promise<void> {
    let currentPolicy: PolicyStruct

    // Fetch networks and nodes in parallel
    const [networksResponse, nodesResponse] = await Promise.all([
        apiClient.get<V1ListUsersResponse>('/user'),
        apiClient.get<V1ListNodesResponse>('/node'),
    ])
    ...
    const exitNodeNetworks = networksWithApprovedExitNodes(nodes)
    const generatedPolicy = generateNetworkIsolationPolicy(networks, exitNodeNetworks)
...
function isAutoGeneratedIsolationRule(rule: ACLRule): boolean {
	...
	if (srcPattern.startsWith('tag:net-')) {
		if (dstPattern === `${srcPattern}:*`) return true
		// Auto-generated exit-node / internet-access rule:
		//   src: ["tag:net-..."]
		//   dst: ["autogroup:internet:*"]
		if (dstPattern === 'autogroup:internet:*') return true
	}
